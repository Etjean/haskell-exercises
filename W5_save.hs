...
findDifference xs ys
    | len_xs /= len_ys = Just (show len_xs ++ " /= " ++ show len_ys)
    | otherwise        = go xs ys
...
-- Ex 9: implement a Num instance for Vector such that all the
...
-- NOTE : Below is my trial at reverse composition and piping
(.>) = flip (.)
($>) = flip ($) -- <= Also exist as "&" in Data.Function

-- freqs :: Ord a => [a] -> [(Int,a)]
-- freqs = sort .> group .> map (\x -> (length x, x!!0))

-- freqs :: Ord a => [a] -> [(Int,a)]
-- freqs xs = xs $> sort $> group $> map (\x -> (length x, x!!0))

freqs :: Eq a => [a] -> [(Int, a)]
freqs [] = []
freqs l@(x:_) = 
    let (left, right) = partition (==x) l
    in (length left, x) : freqs right
...
instance Eq ITree where
  ILeaf == ILeaf                   = True
  INode n1 a1 b1 == INode n2 a2 b2 = n1==n2 && a1==a2 && b1==b2
  _ == _                           = False
...
randomizeTree (Node x t1 t2) g = (Node x' t1' t2', g_final)
    where 
        (x', g')       = random g
        (t1', g'')     = randomizeTree t1 g'
        (t2', g_final) = randomizeTree t2 g''
