...
hFetchLines h nums = do
    ...
    let lines_contents = lines contents
        lines_tokeep = filterIndices nums lines_contents
    return lines_tokeep
        

filterIndices :: [Int] -> [a] -> [a]
filterIndices [] _ = []
filterIndices (i:is) as = (as!!(i-1)) : (filterIndices is as)
...
readCSV path = do
    ...
    let lines = wordsWhen (=='\n') contents
        split_lines = map (wordsWhen (==',')) lines
    return split_lines
...
compareFiles a b = do
    cont_a <- readFile a
    cont_b <- readFile b
    let lines_a = lines cont_a
        lines_b = lines cont_b
        lines_out = helper lines_a lines_b
    mapM_ putStrLn lines_out
...
interact' f state = do
    line <- getLine
    case f (line, state) of 
        (False, s, new_state) -> do 
            putStr s 
            return new_state
        (True, s, new_state) -> do
            putStr s
            interact' f new_state
