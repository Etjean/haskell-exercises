...
countSorted ss = length (filter isOrdered ss)
    where isOrdered s = sort s == s
...
-- funny = map toUpper . intercalate " " . filter ((>5) . length)
...
--    - the elements larger or equal to the pivot are gathered into a list
...
-- Ex 10: powers n max should return all the powers of n that are less
...
sums :: Int -> [Int]
sums 1 = [1]
sums i = sums (i-1) ++ [sumN]
    where sumN = div (i*(i+1)) 2
...
--           go mymax (x:xs)
--             | cmp x mymax == GT = go x xs
--             | otherwise         = go mymax xs



mymaximum :: (a -> a -> Ordering) -> a -> [a] -> a
mymaximum _ def [] = def
mymaximum cmp _ (x:xs) = mymaxlist cmp x xs

mymaxlist _   biggest []     = biggest
mymaxlist cmp biggest (x:xs) = mymaxlist cmp newBiggest xs
    where newBiggest = if cmp x biggest == GT then x else biggest
...
-- interpreter ss = helper 0 0 [] ss

-- helper :: Int -> Int -> [String] -> [String] -> [String]
-- helper _ _ prnt [] = prnt
-- helper a b prnt (s:ss)
--     | s=="incA" = helper (a+1) b prnt ss
--     | s=="decA" = helper (a-1) b prnt ss
--     | s=="incB" = helper a (b+1) prnt ss
--     | s=="decB" = helper a (b-1) prnt ss
--     | s=="printA" = helper a b (prnt++[show a]) ss
--     | s=="printB" = helper a b (prnt++[show b]) ss
--     | otherwise = helper a b prnt ss
...
helper _ _ [] = []
helper a b ("incA":ss) = helper (a+1) b ss
helper a b ("decA":ss) = helper (a-1) b ss
helper a b ("incB":ss) = helper a (b+1) ss
helper a b ("decB":ss) = helper a (b-1) ss
helper a b ("printA":ss) = show a : helper a b ss
helper a b ("printB":ss) = show b : helper a b ss
helper a b (_:ss) = helper a b ss
...
squares :: Int -> [Integer]
squares n = take n . map read . filter isSameNb . map show . map (^2) $ [1..]

isSameNb :: (Eq a) => [a] -> Bool
isSameNb s = head s == last s