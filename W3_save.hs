...
classify es = go es [] []
    where go (Left e: es) as bs = go es (e:as) bs
          go (Right e: es) as bs = go es as (e:bs)
...
insertL x (Node y a b) = Node y (insertL x a) b
...
foldTree f x Leaf = x
foldTree f x (Node n t1 t2) = f n (foldTree f x t1) (foldTree f x t2)
...
rgb Green = [0,1,0]
rgb Blue  = [0,0,1]
rgb (Mix c1 c2) = zipWith (\x y -> min 1 (x+y)) (rgb c1) (rgb c2)
